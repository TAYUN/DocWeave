...
### Requirement: Shared runtime message catalog has a single source of truth

前后端共同依赖的 API 默认错误文案 MUST 由单一共享运行时目录提供，而不是在 `apps/api` 与 `apps/web` 中重复维护。

#### Scenario: frontend and backend reuse the same catalog

- **WHEN** 后端需要把错误码转换为默认中文文案
- **AND** 前端需要基于错误码做兜底展示
- **THEN** 两端都应复用同一份共享目录
- **AND** 维护者不需要在两个应用里分别同步相同文案
