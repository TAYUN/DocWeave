...
export function ChatPage() {
  ...
  const [status, setStatus] = useState<'idle' | 'streaming' | 'completed' | 'cancelled' | 'failed'>(
    'idle'
  )
  ...
  return (
    <Container size={900} px={{ base: 'md', md: 'lg' }} py={{ base: 'md', md: 'xl' }}>
      <Stack gap="lg">
        <div>
          <Title order={1}>知识问答</Title>
          <Text c="dimmed" mt={4}>
            本次回答仅保留在当前页面，不会保存为对话历史。
          </Text>
        </div>
            ...
            <Stack gap="sm">
              ...
              <Group justify="space-between" align="end">
                ...
                {status === 'streaming' ? (
                  ...
                  <Button type="submit" disabled={!message.trim() || indexState !== 'ready'}>
                    开始问答
                  </Button>
                )}
              </Group>
        ...
        {displayView.state === 'index-loading' ? (
          <Alert color="blue" title="正在加载文档索引状态">
            索引状态加载完成后即可开始问答。
          </Alert>
        ) : null}
        {displayView.state === 'index-failed' ? (
          <Alert
            color="red"
            title={indexState === 'failed-index' ? '文档索引失败' : '无法确认文档索引状态'}
          >
            {displayView.errorMessage}
          </Alert>
        ) : null}
        {displayView.state === 'no-index' ? (
          <Alert color="yellow" title="当前范围尚未建立索引">
            完成文档索引后即可进行问答。
          </Alert>
        ) : null}
        {displayView.state === 'restricted' ? (
          <Alert color="red" title="问答范围受限">
            {displayView.errorMessage}
          </Alert>
        ) : null}
        {displayView.state === 'failed' ? (
          <Alert color="red" title="问答失败">
            {displayView.errorMessage}
          </Alert>
        ) : null}
        {displayView.state === 'cancelled' ? (
          <Alert color="gray" title="已停止生成">
            已保留本次已生成的内容和来源。
          </Alert>
        ) : null}
        {displayView.state !== 'restricted' &&
        (displayView.answer ||
          displayView.state === 'streaming' ||
          displayView.state === 'completed' ||
          displayView.state === 'cancelled') ? (
          <Paper withBorder p="md" radius="sm">
            <Stack gap="md">
              <div>
                ...
                <RagAnswer
                  answer={displayView.answer || '正在生成回答...'}
                  citations={displayView.citations}
                />
              </div>
