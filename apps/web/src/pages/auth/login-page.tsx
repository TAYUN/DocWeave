...
export function LoginPage() {
  ...
  return (
    <Box className="auth-layout">
      <Box className="auth-showcase" display={{ base: 'none', lg: 'flex' }}>
        <div className="auth-showcase-content">
          ...
          <Group gap="sm" mb={40}>
            ...
            <Text c="var(--color-ink)" fw={600} size="lg">
              DocWeave
            </Text>
          </Group>

          <span className="info-chip">Knowledge OS</span>
          <Title c="var(--color-ink)" order={1} mt="md" mb="sm">
            让知识流动，
            <br />
            让协作发生。
          </Title>
          ...
          <SimpleGrid cols={2} spacing="md" mt={40}>
            {features.map(({ icon: Icon, label, desc }) => (
              <div key={label} className="soft-panel auth-feature-card">
                <Group gap="sm" mb={4} wrap="nowrap">
                  <div className="feature-icon-box">
                    <Icon size={16} color="var(--color-accent)" />
                  </div>
                  <Text c="var(--color-ink)" fw={600} size="sm">
                    {label}
                  </Text>
                </Group>
                <Text c="var(--color-ink-muted)" size="xs">
                  {desc}
                </Text>
              </div>
            ))}
      ...
      {/* 右侧表单区 — 始终占满 */}
      <Box className="auth-form-shell" ml={{ lg: '42%' }} p={{ base: 24, lg: 48 }}>
        <Group gap="xs" mb={32} hiddenFrom="lg">
          <div className="brand-mark brand-mark--md">
        ...
        >
          <Stack gap="md">
            ...
            {import.meta.env.DEV ? (
              <Paper p="sm" radius="sm" className="dev-account-panel">
                <Stack gap="xs">
                  <Text size="sm" fw={600}>
                    开发账号
                  </Text>
            ...
            {error ? (
              <Alert
                className="status-alert status-alert--error"
                color="red"
