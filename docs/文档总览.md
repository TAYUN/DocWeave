...
9. 文档服务端处理、快照和 RAG 前处理优先采用 `@blocknote/server-util`；导入导出属于第一阶段之后的独立能力线。
...
13. 第一阶段先保留任务抽象和数据库轮询 worker，不急于引入 `BullMQ`；导入、导出、发布和分享不属于 M6 主线。
...
6. M6 RAG 搜索与问答完成后，再分别推进文档输入、文档输出和公开访问能力。
